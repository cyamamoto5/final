...
def search_items():
    ...
    while True:
        ...
        cur = conn.cursor()
        cur.execute('SELECT cg_abb FROM view_items3 WHERE cg_abb=%s', (choose_cg_abb))
    ...
    cur.execute('SELECT id FROM view_items3 WHERE cg_abb=(%s)', (choose_cg_abb))
    collect_choose_id = cur.fetchall()
    for id in collect_choose_id:
        for value in id.values():
            choose_id_list.append(value)

    cur.execute('SELECT item_name FROM view_items3 WHERE cg_abb=(%s)', (choose_cg_abb))
    ...
    cur.execute('SELECT item_detail FROM view_items3 WHERE cg_abb=(%s)', (choose_cg_abb))
    collect_artist_id = cur.fetchall()
    for artist in collect_artist_id:
        for value in artist.values():
            choose_artist_list.append(value)

    cur.execute('SELECT selling_price FROM view_items3 WHERE cg_abb=(%s)', (choose_cg_abb))
    ...
    cur.execute('SELECT storage FROM view_items3 WHERE cg_abb=(%s)', (choose_cg_abb))
...
def shopping():
    ...
    while True:
        shopping_cg_abb_list.clear()
        cur = conn.cursor()
        cur.execute('SELECT cg_abb FROM view_items3 WHERE cg_abb=%s', (add_or_cha_cat))
        add_card_cg_abb= cur.fetchall()
        for cart_cg_abb in add_card_cg_abb:
            for value in cart_cg_abb.values():
                shopping_cg_abb_list.append(value)
    ...
    while True:
        ...
        if add_or_cha_id.isdecimal() == False:
            ...
            if add_or_cha_id == "#" or add_or_cha_id == "＃":
                home()
        cur.execute('SELECT id FROM view_items3 WHERE id=%s and cg_abb=%s', (add_or_cha_id,add_or_cha_cat))
    ...
    if len(more_five_list) == 1 or n <= 4:
        ...
        while True:
            ...
            if item_num.isdecimal() == False:
                ...
                if item_num == "#" or item_num == "＃":
                    home()
            cur.execute('SELECT storage FROM view_items3 WHERE id=%s and cg_abb=%s', (add_or_cha_id, add_or_cha_cat))
        ...
        cur.execute('SELECT item_name, item_detail, selling_price FROM view_items3 WHERE id=%s and cg_abb=%s', (add_or_cha_id, add_or_cha_cat))
...
def insert_order_info():
    ...
    cur.execute('SELECT item_name, storage FROM view_items3 where id=(%s) and cg_abb=(%s)', (order_id1, order_cat1))
    order_item_name1 = cur.fetchall()
    for cus_order_id in order_item_name1:
        for value in cus_order_id.values():
            order_item_name1_list.append(value)

    cur.execute('SELECT item_name, storage FROM view_items3 where id=(%s) and cg_abb=(%s)', (order_id2, order_cat2))
    order_item_name2 = cur.fetchall()
    for cus_order_id in order_item_name2:
        for value in cus_order_id.values():
            order_item_name2_list.append(value)

    cur.execute('SELECT item_name, storage FROM view_items3 where id=(%s) and cg_abb=(%s)', (order_id3, order_cat3))
    order_item_name3 = cur.fetchall()
    for cus_order_id in order_item_name3:
        for value in cus_order_id.values():
            order_item_name3_list.append(value)

    cur.execute('SELECT item_name, storage FROM view_items3 where id=(%s) and cg_abb=(%s)', (order_id4, order_cat4))
    order_item_name4 = cur.fetchall()
    for cus_order_id in order_item_name4:
        for value in cus_order_id.values():
            order_item_name4_list.append(value)

    cur.execute('SELECT item_name, storage FROM view_items3 where id=(%s) and cg_abb=(%s)', (order_id5, order_cat5))
